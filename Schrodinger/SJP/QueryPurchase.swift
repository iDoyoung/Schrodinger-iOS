...
protocol QueryPurchaseProtocol{
    func DetailPurchaseitemDownloaded(items: NSMutableArray , locationcont: Int) // data를 NSArray로 만들겠다! : NS : Next Step - 전통적인 방법
}


class QueryPurchase{
    //jsonmodel 이 portocol 을 가지고 잇어야 함!?
    var delegate: QueryPurchaseProtocol! // data를 받아오는걸 연결하나!? ___ 나
    ...
    func DetailPurchasedownloadItems(pname: String, u_user_no: Int){
        ...
        print("\(pname) QueryPurchase 여기는 이름값이 잘들어갔는지 확인하는 곳")
        print("\(u_user_no) QueryPurchase 여기는 u_user_no값이 잘들어갔는지 확인하는 곳")
        let urlAdd = "?pname=\(pname)&u_user_no=\(u_user_no)"
        
        urlPath = urlPath + urlAdd // urlpath는 진짜 URL
        print("\(urlPath) 여기는 QueryPurchase")
    ...
    func parseJSON(_ data: Data){
        ...
        for i in 0..<jsonResult.count{// 결과를 한 묶음씩 알고있음
               ...
               
            {
                ...
                locations.add(query)
                print("\(query) 여기는 QueryPurchase에서 테스트 ")
        ...
        print("\(locationcont) 여기는 QueryPurchase 집합의 카운트를 세는 곳이야")
