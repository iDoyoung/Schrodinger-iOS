...
protocol QueryThrowProtocol{
    func DetailTrowitemDownloaded(items: NSMutableArray , locationcont: Int) // data를 NSArray로 만들겠다! : NS : Next Step - 전통적인 방법
}


class QueryThrow{
    //jsonmodel 이 portocol 을 가지고 잇어야 함!?
    var delegate: QueryThrowProtocol! // data를 받아오는걸 연결하나!? ___ 나
    ...
    func parseJSON(_ data: Data){
        ...
        for i in 0..<jsonResult.count{// 결과를 한 묶음씩 알고있음
               ...
               
            {
                ...
                locations.add(query)
                print("\(throwDate) 여기는 QueryThrow에서 테스트 ")
        ...
        print("\(locationcont) 여기는 QueryThrow 집합의 카운트를 세는 곳이야")
